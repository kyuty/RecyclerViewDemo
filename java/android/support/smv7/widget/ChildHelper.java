...
class ChildHelper {
     ...
     * @param type     View type, can be {@link android.support.smv7.widget.RecyclerView#INVALID_TYPE}.
    ...
    View findHiddenNonRemovedView(int position, int type) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            android.support.smv7.widget.RecyclerView.ViewHolder holder = mCallback.getChildViewHolder(view);
            if (holder.getLayoutPosition() == position && !holder.isInvalid() && !holder.isRemoved()
                    && (type == android.support.smv7.widget.RecyclerView.INVALID_TYPE || holder.getItemViewType() == type)) {
                return view;
            }
        }
        return null;
